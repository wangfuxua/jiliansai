...
class thumbhandler {
	...
	*/
	function setSrcImg($src_img, $img_type = null) {
		...
		{ 
        	$this->h_src=imagecreatefrompng($src_img);//PNG
		...
		if (empty ( $this->mask_word ) && empty ( $this->mask_img )) 
		{
		    if (($this->img_type == 'png')) 
		    {
	    		...
	    		if (! empty ( $this->img_border_size )) 
	    		{
        	        ...
        	        imagecolortransparent($this->h_dst,$color);
                }*/
		        imagesavealpha($this->h_src,true);
		        $this->h_dst = imagecreatetruecolor($this->dst_w,$this->dst_h);
		        imagealphablending($this->h_dst,false);
		        imagesavealpha($this->h_dst,true);
		        imagecopyresampled ( $this->h_dst, $this->h_src, $this->start_x, $this->start_y, $this->src_x, $this->src_y, $this->fill_w, $this->fill_h, $this->copy_w, $this->copy_h );
	...
	function _setNewImgSize($img_w, $img_h = null) {
		...
		if (2 == $num) {
			...
			switch ($this->cut_type) {
				...
				} else if ($rate_w >= 1  && $rate_h < 1 ) {
					
					/**
					 * 当原图长条状 w>120 h<120 的  截取
					 * 比如 w->200 h->80    
					 * 要的结果  w->120 h->80 在原图上剪切无损
					 */
					$offset = round ( $this->src_w - $fill_w ) / 2;// (200-120)/2 左右偏移
					$this->setSrcCutPosition ( $offset, 0 );//设置偏移
					$this->setRectangleCut ( $fill_w, $this->src_h ); //剪切 $fill_w =120  $this->src_h =80
					$this->copy_w = $fill_w; // 生成图 尺寸  120
					$this->copy_h = $this->src_h;//保持原图高
		
				} else if ($rate_h >= 1 && $rate_w < 1 ) {
					/**
					 * 当原图竖条状 w<120 h>120 的  截取
					 * 比如 w->80 h->200    
					 * 要的结果 w->80 h->120  在原图上剪切无损
					 */
					$offset = round ( $this->src_h - $fill_h ) / 2;
					$this->setSrcCutPosition ( 0, $offset );
					$this->setRectangleCut ( $this->src_w, $fill_h );
					$this->copy_w = $this->src_w;
					$this->copy_h = $fill_h;
