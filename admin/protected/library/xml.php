...
public function arrayxml($array)
{
	...
	$xml="<Request service='OrderService' lang='zh-CN'>
                ...
                <Head>".$array['Head']['0'].",".$array['Head']['1']."</Head><Body>
        ...
        foreach($array['Order'] as $k=>$v){
            ...
            if(isset($array['AddedService'] )){
                foreach($array['AddedService'] as $k=>$v){
                        $xml.=" <AddedService ";
                        foreach($v as $k=>$v){
                         $xml.=" $k='$v' ";
...
public function arrayxml2($array)
{
	$xml="<Request service='RouteService' lang='zh-CN'>
            ...
            <RouteRequest tracking_type='".$array['tracking_type']."'  method_type='".$array['method_type']."' tracking_number='".$array['tracking_number']."' />
...
function file_get_xmlarray($file, $root=null)
{
	if (false === ($contents = @file_get_contents($file))
		|| !($arr = xml2array($contents)))
	{
		return array();
	}
	return $root ? $arr[$root] : $arr;
}
function file_put_xmlarray($file, $array)
{
	$root = key($array);
	$xml = array2xml($array[$root], $root);
	return write_file($file, $xml);
}
